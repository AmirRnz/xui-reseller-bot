...
func HandleAdminSettings(c telebot.Context) error {
	keys := []string{"card_number", "card_owner", "currency_name", "min_topup_amount", "test_reset_days", "support_username", "expiry_notify_days", "ip_limit_factor"}
	...
	text := fmt.Sprintf("⚙️ **Settings**\n\n💳 Card: %s\n👤 Owner: %s\n💱 Currency: %s\n💰 Minimum top-up: %s\n⏱️ Test reset days: %s\n🆘 Support username: %s\n🔔 Expiry notify days: %s\n🌐 IP Limit Factor: %s",
		values["card_number"], values["card_owner"], values["currency_name"], values["min_topup_amount"], values["test_reset_days"], values["support_username"], values["expiry_notify_days"], values["ip_limit_factor"])
	...
	menu.Inline(
		...
		menu.Row(menu.Data("🌐 IP Limit Factor", "admin_set_ip_limit_factor")),
